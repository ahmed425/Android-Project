...
public class SQLAdapter {
    ...
    public long insertNote(String note, int id) {
        ...
        contentValues.put(MySQLHelper.trip_id, id);
    ...
    public long updateNote(String note, String oldNote, int id) {
        ...
        ContentValues contentValues = new ContentValues();
        contentValues.put(MySQLHelper.note, note);
        long i = db.update(MySQLHelper.notes_table, contentValues, MySQLHelper.trip_id +" = " + id + " and " + MySQLHelper.note + " like '" + oldNote + "'", null);
        return i;
    }

    public long deleteNote(String oldNote, int id) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        long i = db.delete(MySQLHelper.notes_table, MySQLHelper.trip_id +" = " + id + " and " + MySQLHelper.note + " like '" + oldNote + "'", null);
        return i;
    }
    ...
    public ArrayList<String> retrieveNotes(int id) {
        ...
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        c = db.query(dbHelper.notes_table, columns, MySQLHelper.trip_id +" = " + id, null, null, null, null);
        while (c.moveToNext()) {
            String note = new String();
            note = (c.getString(0).trim());
            notes.add(note);
        }
        return notes;
    }
    ...
    // Method to retrieve Trips Data from SQLLITE
    ...
    public ArrayList<Trip> StatusTrips(String user,String status) {
        ArrayList<Trip> trips = new ArrayList<Trip>();
        Cursor c;
        String[] args = {user,status};
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        c=db.rawQuery("Select * from "+MySQLHelper.trip_table+" where "+MySQLHelper.user +" = ? and "+MySQLHelper.status+" = ? ",args);
       // Log.i("ic", c.getCount() + "");
        if (c.getCount() > 0) {
            while (c.moveToNext()) {
                Trip trip = new Trip();
    ...
    public long updateStatus(int id, String newSatus)
    {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put(MySQLHelper.status, newSatus);
        long i = db.update(MySQLHelper.trip_table, contentValues, "_id = " + id, null);
        return i;
    }
    ///////////////////////////////////////////////
    ...
    static class MySQLHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL("create table " + notes_table + " (" + trip_id + " integer ," +
                    "" + note + " varchar(255)," + " FOREIGN KEY ("+trip_id+") REFERENCES "+trip_table+"("+id+"));");
        }
